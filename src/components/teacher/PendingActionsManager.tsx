...
const PendingActionsManager: React.FC<PendingActionsManagerProps> = ({
      ...
      try {
        ...
        if (response.ok) {
          ...
          showToast(
            "error",
            t("error"),
            errorData.error || t("failedToLoadPendingActions"),
            5000,
          );
        }
      } catch (error) {
        console.error("Error fetching pending actions:", error);
        showToast("error", t("error"), t("failedToLoadPendingActions"), 5000);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleReviewAction = async (
    ...
    try {
      ...
      showToast(
        "success",
        t("success"),
        `${tAction("action")} ${action === "approve" ? t("approvedSuccessfully") : t("rejectedSuccessfully")}`,
        4000,
      );
      ...
      showToast(
        "error",
        t("error"),
        error instanceof Error ? error.message : `${t("failedTo")} ${action} ${t("action")}`,
        5000,
      );
    } finally {
    ...
    return (
      <div className="card">
        <div className="flex justify-center items-center py-8">
          ...
          <span className="ml-2">{t("loadingPendingActions")}</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="card">
        <div className="py-8 text-center">
          <h3 className="mb-2 font-semibold text-xl">{t("noPendingActions")}</h3>
          <p className="text-gray-600">
            {t("allActionsReviewed")}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="card">
      <div className="mb-6">
        <h3 className="font-bold text-2xl">{t("pendingStudentActions")}</h3>
            ...
            {action.actionType === "Custom" && action.customActionData && (
              <div className="bg-blue-50 mb-4 p-3 border border-blue-200 rounded">
                <h5 className="mb-2 font-medium text-blue-900">
                  {t("customActionDetails")}
                </h5>
                <div className="text-blue-800 text-sm">
                  <div>
                    <strong>{t("effortColon")}</strong> {action.customActionData.effort}
                  </div>
                  <div>
                    <strong>{t("timelineColon")}</strong>{" "}
                    {action.customActionData.timeline} {tAction("years")}
                  </div>
                  <div>
                    <strong>{t("reductionColon")}</strong>{" "}
